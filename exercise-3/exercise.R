...
# and the type to "box" 
plot_ly(midwest, x = ~percollege, y = ~state, type = "box")

# Make two vectors of numbers between 1 and 10, with each vector having 10 numbers in it
num.vector.one <- c(2, 4, 6, 8, 1, 3, 5, 7, 9, 9)
num.vector.two <- c(1, 1, 3, 3, 5, 5, 7, 7, 9, 9)

# create a data frame of the two vectors 
num.data <- data.frame(num.vector.one, num.vector.two)

# now create a basic scatterplot using the data frame you just created, passing it into 
# a variable
basic.scatter <- plot_ly(num.data, x = ~num.vector.one, y = ~num.vector.two, type = "scatter")
...
styled.scatterplot <- plot_ly(num.data, x = ~num.vector.one, y = ~num.vector.two, type = "scatter",
                              marker = list(size = 10, color = 'green', line = list(color = 'blue', width = 2))) %>%
